"""
Tag data AND derive a session key -- key arrives encrypted.

WHEN TO USE   As hrwrequest_ek.py, but where HTTPS alone is not
...
def main():
    ...
    # Ask the cloud for the tag + wrapped key (async: request + poll)
    transactionid = cloud.do_device_EKrequestHRW_rsa(
        credentials.cloudflaretokens, iotaccesstoken, tid, hcw, public_key_pem, False)

    result, hrw_cloud, encrypted_ek = cloud.do_device_EKrequestHRWstatus_rsa(
